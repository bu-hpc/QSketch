...
template <typename Key_T, typename Count_T, typename Hashed_T, typename Seed_T, 
    typename Hash_Function> // 
struct Count_Min_Sketch_GPU_Mem_Levels_Loop : Sketch<Key_T, Count_T> {
    ...
    Count_Min_Sketch_GPU_Mem_Levels_Loop(
        Insert_Kernel_Function *_insert_kernel,
        Search_Kernel_Function *_search_kernel,
        size_t _n_low = default_hash_n,
        size_t _n_high = default_hash_n,
...
// cudaMemcpyToSymbol(constData, data, sizeof(data));
    ...
    virtual ~Count_Min_Sketch_GPU_Mem_Levels_Loop() {
        cudaFree(table_low);
        cudaFree(table_high);
        cudaFree(seed);
        cudaFree(mem_id);
...
template <typename Key_T, typename Count_T, typename Hashed_T, typename Seed_T, 
    typename Hash_Function> // 
struct Count_Min_Sketch_GPU_Mem_Levels_Host_Pre_Cal_Sub_Warp_Loop2 : Sketch<Key_T, Count_T> {
    ...
    Count_Min_Sketch_GPU_Mem_Levels_Host_Pre_Cal_Sub_Warp_Loop2(
        Insert_Kernel_Function *_insert_kernel,
        Search_Kernel_Function *_search_kernel,
        size_t _n_low = default_hash_n,
        size_t _n_high = default_hash_n,
...
// cudaMemcpyToSymbol(constData, data, sizeof(data));
    ...
    virtual int insert(Key_T *keys, size_t keys_sz) {
        ...
        constexpr size_t loop_c = 32;
        size_t mem_sz = ceil<size_t>(n_low, loop_c) * SUB_WARP_SIZE;
        Count_T *htsb = table_low;
        Count_T *htse = table_low + mem_sz;

        // htse = table_low + n_low * SUB_WARP_SIZE;

        // std::cout << n_low << "," << mem_sz << std::endl;
        // int c = 0;

        while (htse <= table_low + n_low * SUB_WARP_SIZE) {
            // std::cout << (c++) << "\t:" << (htsb - table_low) << "--->" << (htse - htsb) << std::endl;
            checkKernelErrors((insert_kernel
                <<<gridDim, blockDim>>>(
                    keys, keys_sz,
                    table_low, n_low,
    ...
    virtual ~Count_Min_Sketch_GPU_Mem_Levels_Host_Pre_Cal_Sub_Warp_Loop2() {
        cudaFree(table_low);
        cudaFree(table_high);
        cudaFree(seed);
        cudaFree(mem_id);
...
template <typename Key_T, typename Count_T, typename Hashed_T, typename Seed_T, 
    typename Hash_Function> // 
struct Count_Min_Sketch_GPU_Mem_Levels_Pre_Load_Host_Pre_Cal_Sub_Warp : Sketch<Key_T, Count_T> {
    ...
    Count_Min_Sketch_GPU_Mem_Levels_Pre_Load_Host_Pre_Cal_Sub_Warp(
        Insert_Kernel_Function *_insert_kernel,
        Search_Kernel_Function *_search_kernel,
        size_t _n_low = default_hash_n,
        size_t _n_high = default_hash_n,
...
// cudaMemcpyToSymbol(constData, data, sizeof(data));
    ...
    virtual ~Count_Min_Sketch_GPU_Mem_Levels_Pre_Load_Host_Pre_Cal_Sub_Warp() {
        cudaFree(table_low);
        cudaFree(table_high);
        cudaFree(seed);
        cudaFree(mem_id);
