...
// __host__ __device__ size_t cal_work_load_r(size_t nwarps, size_t sz, size_t wid) {
//     size_t r = sz % nwarps;
//     size_t t = sz / nwarps;
//     if (wid < r) {
//         ++t;
...
// __host__ __device__ size_t cal_work_load(size_t nwarps, size_t sz, size_t wid) {
//     if (sz % SUB_WARP_NUM == 0) {
//         return cal_work_load_r(nwarps, sz / SUB_WARP_NUM, wid) * SUB_WARP_NUM;
//     }

//     if (wid < nwarps - 1) {
//         return cal_work_load_r(nwarps - 1, sz / SUB_WARP_NUM, wid) * SUB_WARP_NUM;
//     }

//     return sz % SUB_WARP_NUM;
// }
    ...
    void *debug)
{
    ...
    unsigned int sub_warp_id = tid / SUB_WARP_SIZE;
    unsigned int sub_warp_tid = tid % SUB_WARP_SIZE;
    unsigned int sub_warp_mask = 0xff << (sub_warp_id * 8);

    // size_t work_load_per_warp = cal_work_load(nwarps, sz, wid);

    // size_t b = wid * work_load_per_warp + sub_warp_id;
    // size_t e = (wid + 1) * work_load_per_warp;

    size_t b;
    size_t e;

    allocate_work_load(b, e, nwarps, sz, wid);
    ...
    size_t work_load_per_warp = e - b;

    Key_T preload_key;
    if (b < sz) {
        preload_key = keys[b + sub_warp_id];
    }
        
    __shared__ unsigned char result_2_hash_mask[16];
    __shared__ unsigned char result_4_hash_mask[16];

    if (work_load_per_warp % (2 * SUB_WARP_NUM) == 0) {
        size_t i = b + SUB_WARP_NUM;
        size_t sub_warp_preload_sz = 2 * SUB_WARP_NUM;
        size_t ep = e - sub_warp_preload_sz;
        for (; i < ep; i += sub_warp_preload_sz) {
            Key_T result_0_key = keys[i + sub_warp_id]; 
            ...
            cal1_pre_cal_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                ...
                hash,
                seed, s_sz,
                tid, sub_warp_id, sub_warp_tid, sub_warp_mask, preload_key
            );

            // preload_key = keys[i + SUB_WARP_NUM + sub_warp_id];

            Count_T result_2_htl_7 = result_1_htl[7];
            Count_T result_2_cv = result_1_htl[sub_warp_tid];
            if (tid % 2 == 0) {
                result_2_hash_mask[tid / 2] = hash_mask_table[result_1_hash_mask_id + sub_warp_tid / 2];    
            }
            ...
            cal1_pre_cal_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                ...
                hash,
                seed, s_sz,
                tid, sub_warp_id, sub_warp_tid, sub_warp_mask, result_0_key
            );

            Count_T result_4_htl_7 = result_3_htl[7];
            Count_T result_4_cv = result_3_htl[sub_warp_tid];
            if (tid % 2 == 0) {
                result_4_hash_mask[tid / 2] = hash_mask_table[result_3_hash_mask_id + sub_warp_tid / 2];    
            }

            // __threadfence_block();
            cal2_pre_call_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                result_2_cv, hash_table_high, mem_id,
                result_1_htl, result_2_htl_7, result_2_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, preload_key);
            preload_key = keys[i + SUB_WARP_NUM + sub_warp_id];

            cal2_pre_call_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                result_4_cv, hash_table_high, mem_id,
                result_3_htl, result_4_htl_7, result_4_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, result_0_key);
    ...
    } else if (work_load_per_warp % (2 * SUB_WARP_NUM) == SUB_WARP_NUM) {
        size_t i = b + SUB_WARP_NUM;
        size_t sub_warp_preload_sz = 2 * SUB_WARP_NUM;
        size_t ep = e - sub_warp_preload_sz;
        for (; i < ep; i += sub_warp_preload_sz) {
...
template <typename Key_T, typename Count_T, typename Hashed_T, typename Seed_T, typename Hash_Function>
__global__ void search_warp_mem_threadfence_preload_pre_cal_sub_warp(Key_T *keys, size_t sz,
    Count_T *hash_table_low, size_t n_low,
    Count_T *hash_table_high, size_t n_high, unsigned int *mem_id,
    const Hash_Function &hash,
    Seed_T *seed, size_t s_sz, 
    ...
    void *debug)
{
    ...
    //     printf("search_warp_mem_threadfence_preload_pre_cal_sub_warp\n");
    // }
    ...
    if (work_load_per_warp % (2 * SUB_WARP_NUM) == 0) {
        ...
        for (; i < ep; i += sub_warp_preload_sz) {
            ...
            //     result_1_htl, result_2_htl_7, result_2_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, preload_key);
            search_calculation_2_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                hash_table_high, count + i - SUB_WARP_NUM + sub_warp_id, result_2_cv,
                result_2_htl_7, result_2_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, preload_key);
            // if (tid == 0) {
            //     printf("i : %lu p3\n", i);
            // }
            ...
            //     result_3_htl, result_4_htl_7, result_4_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, result_0_key);
            search_calculation_2_sub_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                hash_table_high, count + i + sub_warp_id, result_4_cv,
                result_4_htl_7, result_4_hash_mask, tid, sub_warp_id, sub_warp_tid, sub_warp_mask, result_0_key);
            // if (tid == 0) {
            //     printf("i : %lu p4\n", i);
            // }
        }
