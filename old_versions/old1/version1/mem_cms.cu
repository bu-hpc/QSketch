...
__global__ void insert2_old(T *keys, size_t sz,  size_t work_load_per_warp,
    C *hash_table, size_t n, size_t m,
    T *seed, size_t s_sz, 
    void *debug)
{
...
int main(int argc, char const *argv[])
{
...

    {
        ...
        checkKernelErrors((insert<<<nblocks, threads_per_block>>>(keys, keys_sz, work_load_per_warp,
             hash_table, n, m, seed, s_sz, nullptr)));
        ...
        std::cout << "insert perf: " << double(insert_number) / time_span.count() / (1ul<<20) << "Mops/s" << std::endl;
...

    {
        ...
        checkKernelErrors((insert2<<<nblocks, threads_per_block>>>(keys, keys_sz, work_load_per_warp,
             hash_table, n, m, seed, s_sz, nullptr)));
        ...
        size_t insert_number = keys_sz * insert_loop;
        // std::cout << "total_bytes: " << total_bytes << "-->" << (total_bytes / (1ul<<30)) << std::endl;
        std::cout << "insert2 perf: " << double(insert_number) / time_span.count() / (1ul<<20) << "Mops/s" << std::endl;
