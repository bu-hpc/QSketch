...
struct Test {
    ...
    int search_accuracy_test(size_t insert_keys_sz, size_t search_keys_sz,
     size_t search_loop = 1, std::ostream &os = std::cout) {
        ...
        // unsigned char *hb1 = new unsigned char[insert_keys_sz * HASH_MASK_SIZE_SUB_WARP];
        // unsigned char *hb2 = new unsigned char[insert_keys_sz * 16];
        // size_t *hdebug = new size_t[insert_keys_sz];

        {
            // checkKernelErrors(cudaMemcpy(hb1, db1, insert_keys_sz * HASH_MASK_SIZE_SUB_WARP, cudaMemcpyDeviceToHost));
            // checkKernelErrors(cudaMemcpy(hdebug, debug, insert_keys_sz * sizeof(size_t), cudaMemcpyDeviceToHost));
            
            // for (size_t j = 0; j < insert_keys_sz * HASH_MASK_SIZE_SUB_WARP; ++j)
            // {
            //     std::cout << hb1[j] << std::e
            // }
            // std::cout.write(hb1, insert_keys_sz * HASH_MASK_SIZE_SUB_WARP);
        }
        ...
        for (size_t i = 0; i < search_loop; ++i) {
            ...
            if (true){
                ...
                checkKernelErrors(cudaMemcpy(h_counts, counts, search_keys_sz * sizeof(Count_T), cudaMemcpyDeviceToHost));
                checkKernelErrors(cudaMemcpy(h_e_counts, e_counts, search_keys_sz * sizeof(Count_T), cudaMemcpyDeviceToHost));
                checkKernelErrors(cudaMemcpy(h_keys, keys, insert_keys_sz * sizeof(Key_T), cudaMemcpyDeviceToHost));
                ...
                // for (size_t j = 0; j < insert_keys_sz; ++j) {
                //     std::cout << h_keys[j] << std::endl;
                // }
            }
