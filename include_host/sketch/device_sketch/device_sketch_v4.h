    ...
    void *debug)
{
    ...
    for (size_t i = b + tid; i < e; i += warpSize) {
        ...
        uint *index_hash_mask = index_hash_mask_table + id * index_hash_mask_sz;

        insert(v, insert_bucket, search_bucket, 
...
struct Sketch_GPU_SF_Sub_Warp : public Sketch_GPU_Warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function> {
    ...
    Sketch_GPU_Sub_Warp(size_t _n, size_t _m, uint times,
