...
Percentage_T average_diff(thrust::device_vector<C> &diff) {
    Percentage_T sum = thrust::reduce(diff.begin(), diff.end(), 0.0f, thrust::plus<C>());
    return sum / diff.size();
}

template <typename C>
C sum(thrust::device_vector<C> &diff) {
    return thrust::reduce(diff.begin(), diff.end(), C(0), thrust::plus<C>());
}
...
template <typename C>
size_t greater(thrust::device_vector<C> &diff) {
    size_t count = thrust::count_if(diff.begin(), diff.end(), Count_Greater<C>());
