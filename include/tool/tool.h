...
struct GPU_Tools : CPU_Tools<T> {
    ...
    T *zero_managed(T *keys, size_t keys_sz) {
        if (keys == nullptr) {
            CUDA_CALL(cudaMallocManaged(&keys, sizeof(T) * keys_sz));
        }
        cudaMemset(keys, 0, sizeof(T) * keys_sz);
        return keys;
    }

    void zero_zero_copy_memory(T **host_table_p, T **table_p, size_t sz) {
        ...
        if (*host_table_p == nullptr) {
            ...
            #ifdef QSKETCH_DEBUG
                std::cout << "cudaHostAlloc: " << sizeof(T) * sz << "byte" << std::endl;
            #endif

            CUDA_CALL(cudaHostAlloc(host_table_p, sizeof(T) * sz, cudaHostAllocMapped));
            CUDA_CALL(cudaHostGetDevicePointer(table_p, *host_table_p, 0));
            
            // printf("table_p: %lu\n", *table_p);
            // CUDA_CALL(cudaDeviceSynchronize());
        }
        CUDA_CALL(cudaMemset(*table_p, 0, sizeof(T) * sz));
    }
