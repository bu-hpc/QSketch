...
#endif
#define QSKETCH_KERNEL_NAMESPACE Sketch_GPU_Sub_Lock_Warp_Level_Kernel
...
template <typename Key_T, typename Count_T, typename Hashed_T, typename Seed_T, typename Hash_Function>
__device__ inline void calculation_s2_insert_warp(
    
    const Count_T &cv,
    Count_T htl_next_level_id, // htl_7
    ...
    ) 
{
    ...
    Count_T id = __shfl_sync(0xffffffff, htl_next_level_id, valid_sid * SUB_WARP_SIZE);
    if (id > default_values::DEVICE_NEXT_LEVEL_ID_START) {
        uchar *hmh = hash_mask + valid_sid * hash_mask_sz; // hmh: hash_mask_high
    ...
    if (update_low_warp) {
        ...
        if (tid % SUB_WARP_SIZE == 0 && update_low) {
            ...
            htl_next_level_id = id;
        }
        __threadfence_block();
        Count_T idv = __shfl_sync(0xffffffff, htl_next_level_id, valid_sid * SUB_WARP_SIZE);
    ...
    if (stid == 0) {
        // printf("write: tid: %lu, htl_next_level_id: %u\n", tid, htl_next_level_id);
        atomicExch(htl + dht_low.m - 1, htl_next_level_id);
    }
}
...
// printf("tid: %lu\n", tid);
    uint stid0_mask = 0;
    if (m_low == 8) {
        stid0_mask = 0 | 1u | (1u << 8) | (1u << 16) | (1u << 24);
    }
    if (m_low == 16) {
        stid0_mask = 0 | 1u | (1u << 16);
    }
    // printf("tid: %lu\n", tid);
    const uint ma_single = (1u << tid);
    __syncwarp();
...
// printf("tid: %lu\n", tid);
    ...
    for (size_t i = b + sid; i < e; i += sub_warp_num) {
        ...
        // size_t htl_c = static_cast<size_t>(htl);
        size_t htl_c = reinterpret_cast<size_t>(htl);
        // size_t htl_c = htl;
        // size_t htl_f = __shfl_sync(stid0_mask, htl_c, (tid + m) % warpSize);

        uint ma = __match_any_sync(stid0_mask, htl_c);
        uint ma_r = __any_sync(0xffffffff, ma == ma_single);
        // printf("tid: %lu, ma_r: %u\n", tid, ma_r);
        if (ma_r) {
            // sequencial insert
            // if (tid == 0) {
            //     printf("start\n");
            ...
            if (stid == 0) {
                // printf("read: htl: %lu, tid: %lu\n", htl, tid);
                // printf("ma_r: %x\n", ma_r);
            }
            
            for (uint vsid = 0; vsid < sub_warp_num ; vsid++) {
                Count_T htl_next_level_id = 1;
                if (sid == vsid && stid == 0) {
                    while (htl_next_level_id == 1) 
                    {
                        htl_next_level_id = atomicExch(htl + m_low - 1, 1);
                        // printf("read: htl: %lu, tid: %lu, htl_next_level_id: %u\n", htl, tid, htl_next_level_id);
                        __threadfence_block();
                    }
                }
                __syncwarp();
                htl_next_level_id = __shfl_sync(smask, htl_next_level_id, sid * m_low);
                __threadfence_block();

                calculation_s2_insert_warp<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function>(
                    cv,
...
struct Sketch_GPU_Sub_Lock_Warp_Level : public Sketch_GPU<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function> {
    ...
    Sketch_GPU_Sub_Lock_Warp_Level(size_t _n_low, size_t _m_low,
    ...
    {        
#ifdef QSKETCH_DEBUG
        std::cout << "Sketch_GPU_Sub_Lock_Warp_Level" << (_m_low) << std::endl;
