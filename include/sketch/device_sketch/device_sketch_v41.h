...
#endif
#define QSKETCH_KERNEL_NAMESPACE Sketch_GPU_SF_Managed_Sub_Warp_Kernel
...
__global__ void insert_kernel(Key_T *keys, size_t sz,
    ...
    Managed_Hash_Table<Count_T> dht_insert,
...
struct Sketch_GPU_SF_Managed_Sub_Warp : public Sketch_GPU<Key_T, Count_T, Hashed_T, Seed_T, Hash_Function> {
    ...
    Sketch_GPU_SF_Managed_Sub_Warp(size_t _n, size_t _m, uint _times,
...
#ifdef QSKETCH_DEBUG
        std::cout << "Sketch_GPU_SF_Managed_Sub_Warp" << (_m) << std::endl;
...
//     void *debug)
    ...
    virtual ~Sketch_GPU_SF_Managed_Sub_Warp() {
        // cudaFree(table);
        // cudaFree(seed);
        dht_search.free();
        dht_insert.free();
