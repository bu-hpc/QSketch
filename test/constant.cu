...
template<typename T>
__global__ void test2(unsigned long long *ans) {
    size_t tid = threadIdx.x;
    size_t sum = 0;
    for (unsigned int i = 0; i < WORKLOAD_PERTHREAD; ++i)
    {
...
int main(int argc, char const *argv[])
{
    ...
    unsigned long long hans;
    cudaMemcpy(&hans, &ans, sizeof(unsigned long long), cudaMemcpyDeviceToHost);
    std::cout << "ans: " << hans << std::endl;
